...
static int32_t Receive_Packet(uint8_t *data, int *length, uint32_t timeout)
{
	...
	*data = (uint8_t) ch;
	...
	//Ensure packet data is cleared first
	memset(dptr, 0, count);
...
//	int nfile = 1;
	char *temp_path = "/sdcard/temp.ymodem";
	char f_name[100];
	char f_path[100];
	// ==== Receive file ====
	uint32_t max_fsize = 50000000;

	console_task_pause();

	// Open the file
	ffd = fopen(temp_path, "wb");
	if (ffd) {
		ESP_LOGI(TAG, "Receiving file, please start YModem transfer on host ...\r\n");
		rec_res = Ymodem_Receive(ffd, max_fsize, f_name);
		fclose(ffd);

		if (rec_res > 0) {
			ESP_LOGI(TAG, "Received file %s", f_name);
			sprintf(f_path, "/sdcard/%s", f_name);
			ESP_LOGI(TAG, "Transfer complete, Size=%d, orig name: \"%s\"", rec_res, f_path);
			ESP_LOGD(TAG, "Old file %s New file %s", temp_path, f_path);
			remove(f_path);
			rename(temp_path, f_path);

			led_eye_pulse_stop();
			lolcode_execute(f_path, true);
		}
		else {
			ESP_LOGE(TAG, "Ymodem timeout.");
			remove(temp_path);
		}
	}
	else {
		ESP_LOGE(TAG, "Error opening file \"%s\" for receive.", temp_path);
	}
