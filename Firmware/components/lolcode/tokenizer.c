...
static const char *TAG = "LULZ::Tokenizer";
...
int isInteger(const char *image)
{
	...
	if (*cur == '-'
			|| (isdigit((unsigned char)*cur) && *cur != '0')
			|| (*cur == '0' && *(cur + 1) == '\0')) {
		cur++;
...
{
	const char *cur = image;
	if (*cur == '-' || isdigit((unsigned char )*cur)) {
		cur++;
		while (isdigit((unsigned char )*cur))
			cur++;
		if (*cur == '.') {
...
int isIdentifier(const char *image)
{
	...
	if (!cur || !isalpha((unsigned char )*cur))
		return 0;
	cur++;
	while (*cur) {
...
Token *createToken(TokenType type,
		...
		unsigned int line)
{
	Token *ret = util_heap_alloc_ext(sizeof(Token));
	if (!ret) {
 ...
 */
int addToken(Token ***list, unsigned int *num, Token *token) {
	unsigned int newsize = *num + 1;

	void *mem = util_heap_realloc_ext(*list, sizeof(Token *) * newsize);
	if (!mem) {
		ESP_LOGE(TAG, "%s[%d]: Unable to add a '%s' token. realloc failed. Old size=%d New size=%d. Free Heap=%d",
				token->fname,
				token->line,
				token->image,
				*num,
		...
		unsigned int *start)
{
	...
	for (type = 0; type != TT_ENDOFTOKENS; type++) {
		...
		unsigned int num = acceptLexemes(lexemes,
				*start, lulzcode_keywords[type]);
		if (!num)
			continue;
		/* If so, create a new token for the keyword */
		token = createToken(type, lulzcode_keywords[type], fname, line);
...
Token **tokenizeLexemes(LexemeList *list)
{
	...
	for (n = 0; n < list->num; n++) {
		...
		/* EOF */
			...
			lulz_error(TK_UNKNOWN_TOKEN, fname, line, image);
			/* Clean up */
	...
	mem = util_heap_realloc_ext(ret, sizeof(Token *) * ++retsize);
	if (!mem) {
